...
for i=1:size(point1,2)
    ...
    mi_1=[point1(:,i);1];
    mi_1t=mi_1';% faccio la trasposta
    % prodotto di kronecker
    kro=kron(mi_1t, prod_esterno);
    % Prendo le prime due righe linearmente indipendenti
    A=[A; kro(1,:);kro(2,:)];%avrei 3 righe ma solo due sono linearmente indipendenti
end

% Calcolo di H
h = ns(A);% risolve il null-space problem A*v =0
%però ora h è un vettore

H = reshape(h,3,3);
H=H./H(3,3); %non è chiaro, comunque vale per un fattore di scala
%quindi anche se divido per l'ultimo elemento è corretto ?
...
% Ouput:
mosaic_out=mosaic_ref.*0.5+mosaic_mov.*0.5;%che pixel prendo nella parte con overlap?
%in questa strategia ho fatto la somma della metà del valore
